...
use emma65::emulator::{
    BuildError, BusConfig, Config, CpuVariantSpec, DeviceModule, DeviceModuleError, DeviceRegistry,
    InstantiationContext, RamModule, RomModule,
};
...
async fn ram_module_plain() {
    ...
    RamModule
        .instantiate(BusConfig::new(), 0x2000, &a, &ctx(), id_allocator)
        .await
        .unwrap();
}
...
async fn ram_module_with_fill() {
    let id_allocator = Arc::new(Mutex::new(DeviceIdAllocator::new()));
    let a = attrs(&[
        ("size", Value::from(1024u32)),
        ("fill", Value::from(0xABu32)),
    ]);
    let bus_config = RamModule
        .instantiate(BusConfig::new(), 0x2000, &a, &ctx(), id_allocator)
        .await
        .unwrap();
...
async fn ram_module_with_binary_image() {
    ...
    let bus_config = RamModule
        .instantiate(BusConfig::new(), 0x3000, &a, &ctx(), id_allocator)
        .await
        .unwrap();
    let mut bus = bus_config.build();
...
async fn ram_module_with_intel_hex_image() {
    ...
    let bus_config = RamModule
        .instantiate(BusConfig::new(), 0x0000, &a, &ctx(), id_allocator)
        .await
        .unwrap();
    let mut bus = bus_config.build();
...
async fn registry_instantiates_ram() {
    ...
    registry
        .instantiate("ram", BusConfig::new(), 0x1000, &a, &ctx(), id_allocator)
        .await
        .unwrap();
}
...
async fn registry_instantiates_rom() {
    ...
    registry
        .instantiate("rom", BusConfig::new(), 0x8000, &a, &ctx(), id_allocator)
        .await
        .unwrap();
}
...
async fn registry_unknown_module_name() {
    ...
    let a = attrs(&[]);
    let err = registry
        .instantiate("bogus", BusConfig::new(), 0x1000, &a, &ctx(), id_allocator)
        .await
        .err()
        .unwrap();
