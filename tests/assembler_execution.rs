...
use emma65::assembler::{AssembledProgram, assemble};
use emma65::emulator::cpu::StepResult;
use emma65::emulator::{
    AddressRange, Bus, ClockSpeed, CpuBuilder, CpuVariant, InvalidOpcodePolicy,
};
...
";
    ...
    assert_eq!(
        cpu.registers().a,
        100,
        "A should hold the summed total (10+20+30+40)"
    );
    assert_eq!(
        cpu.registers().x,
        4,
        "X should have counted through all 4 table entries"
    );
    assert_eq!(
        cpu.registers().y,
        1,
        "Y should have been incremented once by the subroutine"
    );
    assert_eq!(
        cpu.bus().peek(0x0010).unwrap(),
        100,
        "result byte should hold the summed total"
    );
}
...
";
    let (mut cpu, program) = load_program(source);
    let irq_handler = program
        .symbols
        .address_for("irq_handler")
        .expect("irq_handler symbol should be defined");
    cpu.bus_mut()
        .write(0xFFFE, (irq_handler & 0xFF) as u8)
        .unwrap();
    cpu.bus_mut()
        .write(0xFFFF, (irq_handler >> 8) as u8)
        .unwrap();
    let start = program.segments[0].origin;
    reset_at(&mut cpu, start);
    ...
    assert_eq!(
        cpu.registers().a,
        0x99,
        "LDA #$99 should have executed intact after the BRK/RTI round trip"
    );
}
