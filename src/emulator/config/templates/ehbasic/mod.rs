//! Bundled EhBASIC starter-profile template: EhBASIC ROM
//! image, its VICE labels file, and a device-layout template (RAM, ROM, VIA,
//! console). Registered as the `"ehbasic"` starter-profile template in
//! [`super`](super).
...
mod tests {
    ...
    fn materialize_renders_a_config_that_round_trips_through_figment() {
        ...
        let config: crate::emulator::Config = Figment::new()
            ...
            .extract()
            .expect("bundled ehbasic config failed to parse");
