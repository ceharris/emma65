...
Wire protocol the [Character Display](io-devices.md#character-display-display)
...
[LED Matrix External Protocol](appendix-led-matrix-protocol.md) — a different
...
[Character Display](io-devices.md#character-display-display) for the
...
The char/color RAM sent is always the buffer currently on screen — the
scanout buffers in double-buffered mode, the CPU-addressable buffers
directly otherwise — i.e. exactly the same data the debugger's in-process
compositing reads on the same vsync.
