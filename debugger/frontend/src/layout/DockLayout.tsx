...
function addDefaultLayout(
  ...
  const groupMembers: { id: MainPanelId; detached: boolean }[] = [
  ...
  if (present.length > 0) {
    ...
    // width) to render at a legible integer scale by default — see DisplayPanel.tsx. This rule
    // depends only on whether Display is present, not on which member ends up as the anchor.
    add(anchor.id, { initialWidth: displayDetached ? 592 : 660 });
    for (const member of rest) {
      add(member.id, { position: { referencePanel: anchor.id } });
    }
    add("memory", { position: { referencePanel: anchor.id } });
  } else {
...
function addMissingBottomPanels(
  ...
  if (!hasLedMatrix && !ledMatrixDetached) {
    api.addPanel({
      id: "led-matrix",
...
function makeDockTabActions(
  ...
  return function DockTabActions({ activePanel, containerApi }: IDockviewHeaderActionsProps) {
    ...
    if (activePanel?.id === "led-matrix") {
      const handleDetach = () => {
        invoke("detach_led_matrix")
...
export default function DockLayout() {
  ...
  // LED matrix device plan, Work Unit 5) — falls back to its default
  // position tabbed with Memory/Terminal/Display
  // (`DEFAULT_PANEL_POSITION["led-matrix"]`) when there's no resolvable
  // remembered position.
  useEffect(() => {
